...
-- Consulta: Cantidad ahorrada, el monto del prestamo y el garante de las personas cuyo prestamo está entre $100 y $1000.
select 
...
-- SubConsulta: datos de la persona cuya cédula sea igual a la cédula del nombre “Sean”
select 
    pe.cedula,
    pe.nombre,
    pe.apellido,
...
select
    pr.cedula,
    SUM(cast(pr.monto as numeric)) as total_prestamos
from 
    prestamo pr
group by 
    pr.cedula;
